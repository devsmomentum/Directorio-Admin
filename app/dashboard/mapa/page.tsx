...
export default function MapaEditorPage() {
  ...
  const handleUploadBg = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { const fp = `plano_${FLOOR_LABELS[selectedFloor].toLowerCase()}.png`; const { error } = await supabase.storage.from('mapas').upload(fp, file, { upsert: true, cacheControl: '0' }); if (error) throw error; const { data } = supabase.storage.from('mapas').getPublicUrl(fp); if (data?.publicUrl) loadBgFromUrl(data.publicUrl + '?t=' + Date.now()); showToast('Plano actualizado'); } catch (err: any) { alert('Error: ' + err.message); }
    finally { setUploadingBg(false); if (bgFileRef.current) bgFileRef.current.value = ''; }
  };
  const handleRemoveBg = async () => { if (!confirm('Quitar imagen de fondo?')) return; await supabase.storage.from('mapas').remove([`plano_${FLOOR_LABELS[selectedFloor].toLowerCase()}.png`]); bgImageRef.current = null; setHasBgImage(false); cam.current = { x: 0, y: 0, zoom: 1 }; setZoomLabel('100%'); draw(); showToast('Plano eliminado'); };
  ...
  const handleDeletePoly = async (id: string) => { if (!confirm('Eliminar esta area?')) return; await supabase.from('map_polygons').delete().eq('id', id); setSelectedPolyId(null); showToast('Area eliminada'); fetchData(); };
  ...
  const handleDeleteRoute = async (id: string) => { if (!confirm('Eliminar esta ruta?')) return; await supabase.from('map_routes').delete().eq('id', id); setSelectedRouteId(null); showToast('Ruta eliminada'); fetchData(); };
