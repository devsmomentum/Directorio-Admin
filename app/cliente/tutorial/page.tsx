    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <section className="bg-[#111] border border-white/5 rounded-2xl p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-white/30 uppercase tracking-widest font-medium">Sección 3</p>
            <h3 className="text-lg font-bold text-white">Smart Kill-Switch &amp; Cronograma de Corte</h3>
          </div>
        </div>

        <p className="text-white/60 text-sm">
          El sistema opera bajo la lógica de <span className="text-red-300 font-semibold">"Policía Digital"</span>:
          implacable pero justo. La intervención manual en la desconexión por impago es cero.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-4">
            <p className="text-amber-300 text-sm font-bold mb-1">⏰ Aviso 3 días antes</p>
            <p className="text-white/60 text-xs">
              Recibes notificación 3 días antes de la fecha de corte para que reportes tu pago a tiempo.
            </p>
          </div>
          <div className="bg-orange-500/5 border border-orange-500/20 rounded-lg p-4">
            <p className="text-orange-300 text-sm font-bold mb-1">🌙 Día de corte</p>
            <p className="text-white/60 text-xs">
              Si a las 00:00 del día posterior al vencimiento no existe pago registrado, el Ad-Server elimina automáticamente tu video del loop de las pantallas.
            </p>
          </div>
          <div className="bg-red-500/5 border border-red-500/20 rounded-lg p-4">
            <p className="text-red-300 text-sm font-bold mb-1">📲 Notificación</p>
            <p className="text-white/60 text-xs italic">
              "Tu pauta ha sido pausada por falta de pago. Reactívala aquí +58 412-5570011".
            </p>
          </div>
        </div>
        ...
        <div className="bg-white/[0.02] border border-white/5 rounded-lg p-4">
          <p className="text-white/70 text-xs leading-relaxed">
            <span className="text-white font-semibold">Calendario de Rotación:</span> El sistema calcula
            automáticamente el fin de tu pauta según el plan: 5 días exactos para el plan semanal, 30 días
            para el mensual. Tras la confirmación de pago vía Bancamiga / Binance / Efectivo, la pauta se
            reactiva en el siguiente ciclo del loop.
          </p>
        </div>
      </section>

      {/* ── 4. Engagement / Flash Coupons (extra del PDF) ── */}
      <section className="bg-[#111] border border-white/5 rounded-2xl p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-white/30 uppercase tracking-widest font-medium">Sección 4</p>
            <h3 className="text-lg font-bold text-white">Engagement: Flash Coupons &amp; SuperAPI</h3>
          </div>
        </div>

        <p className="text-white/60 text-sm">
          No queremos que el usuario solo pase por delante. Usamos la <span className="text-pink-300 font-semibold">SuperAPI</span> para
          que la IA "invite" a la gente a interactuar:
        </p>

        <blockquote className="border-l-2 border-pink-500/50 pl-4 italic text-pink-200 text-sm">
          "¿Buscas zapatos? Toca aquí y llévate un cupón de 30 segundos."
        </blockquote>

        <p className="text-white/50 text-xs">
          La urgencia del <span className="text-pink-300 font-semibold">Flash Coupon</span> es nuestra mejor métrica de éxito.
          Vincula tu plan a un cupón relámpago y multiplica el engagement.
        </p>
      </section>
